...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function applogin($login, $password)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			if (!is_null($user = $this->ci->Users->get_user_by_username($login))) 
			{
				...
				{
					return json_encode(array("success"=>1,"message"=>"Logged In Successfully","staff_id"=>$user->id,"hotel_id"=>$user->hotel_id));
				}
				else
				{
					return json_encode(array("success"=>0,"message"=>"Wrong Password"));
				}
			}
			else
			{
					return json_encode(array("success"=>0,"message"=>"Wrong Username"));
			}
		}
		else
		{
			return json_encode(array("success"=>0,"message"=>"OOPS ! Somehing Went Wrong"));
		}
	}
	 ...
	 * @return	array
	 */
	function addstaff($name,$username,$password,$hotel)
	{
	...
	function create_user($username,$name,$gender,$email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->Users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
	 ...
	 * @return	bool
	 */
	function res_pass($user_id,$new_password)
	{
		if ((strlen($user_id) > 0)  AND (strlen($new_password) > 0)) {
			if (!is_null($user = $this->ci->Users->get_user_by_id($user_id, TRUE))) {
				...
				$this->ci->Common_model->update("users",array("password"=>$hashed_password),array("id"=>$user_id));
				return TRUE;
			}
		}
		return NULL;
